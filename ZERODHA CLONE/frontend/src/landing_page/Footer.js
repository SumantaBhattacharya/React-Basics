...
function Footer() {
  ...
  const linkStyle = (index) => ({
    ...
    fontWeight: "600",
    fontSize: "1rem",
    // Changed display to block + width fit-content so only the text is clickable
    display: "block", 
    width: "fit-content",
    marginBottom: "5px",
    transition: "color 0.3s ease-in-out",
    cursor: "pointer", // Ensures pointer only appears on the link
  });

  // Define company links with their routes
  const companyLinks = [
  ...
  // Define support links with their routes
  const supportLinks = [
    { name: "Contact", path: "#" }, 
    { name: "Support portal", path: "/support" },
    { name: "Z-Connect blog", path: "#" }, 
    { name: "List of charges", path: "#" }, 
    { name: "Downloads & resources", path: "#" } 
  ];

  // Define account links with their routes
  const accountLinks = [
